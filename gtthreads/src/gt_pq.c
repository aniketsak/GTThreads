...
extern uthread_struct_t *sched_find_best_uthread(kthread_runqueue_t *kthread_runq)
{
    ...
    //AOS
    //printf("NUMBER OF CPUS = %d",num_cpus);
    //AOS end
	...
	int my_count = kthread_runq->switch_counter;
	...
	{ /* No jobs in active. switch runqueue */
        ...
        if(sched_policy==1) {
            ...
            int i;
            for (i = 0; i < num_cpus; i++) {      //4 instead of num_cpus
                if (i == kthread_apic_id())
					...
					if((my_count >= k_ctx->krunqueue.switch_counter)) {
						if (!(k_ctx->krunqueue.active_runq->uthread_mask)) {
							gt_spin_unlock(&(k_ctx->krunqueue.kthread_runqlock));
							continue;
						} else {
							uprio = LOWEST_BIT_SET(k_ctx->krunqueue.active_runq->uthread_mask);
							prioq = &(k_ctx->krunqueue.active_runq->prio_array[uprio]);
							...
							__rem_from_runqueue(k_ctx->krunqueue.active_runq, u_obj);
