...
extern void uthread_schedule(uthread_struct_t * (*kthread_best_sched_uthread)(kthread_runqueue_t *))
{
	...
	if((u_obj = kthread_runq->cur_uthread))
	{
		...
		u_obj->total_time += end_time.tv_sec*1000000 + end_time.tv_usec - (u_obj->start_time.tv_sec*1000000) - (u_obj->start_time.tv_usec);
        temp = end_time.tv_sec*1000000 + end_time.tv_usec - (u_obj->start_time.tv_sec*1000000) - (u_obj->start_time.tv_usec);
		if(temp/KTHREAD_VTALRM_USEC >= 1)   // to account for minor overflows in time calculations
		{
			u_obj->credit -= 25;
		}
		else{
			u_obj->credit -= (temp*25/KTHREAD_VTALRM_USEC);
		}
		if(u_obj->credit <= 0){
			u_obj->credit_flag =1;
		}
		...
		{    //printf("\nTHREAD ID = %d   TOTAL TIME =    %lu   \n",u_obj->uthread_tid,u_obj->total_time);
			/* XXX: Inserting uthread into zombie queue is causing improper
			 * cleanup/exit of uthread (core dump) */
			execution_time[u_obj->uthread_tid] = u_obj->total_time;
		...
		else
		{
            ...
            {
                if(u_obj->credit_flag == 0)
                {
//                    u_obj->credit = u_obj->credit - 25;
//					if(u_obj->credit < 0)
//						u_obj->credit_flag = 1;
...
extern void gt_yield()
{
	...
	kthread_context_t *k_ctx;
	k_ctx = kthread_cpu_map[kthread_apic_id()];
	printf("GT_YIELD CALLED BY THREAD %d \n",k_ctx->krunqueue.cur_uthread->uthread_tid);
	gettimeofday(&tv2,NULL);
	struct timeval tv1;
	tv1 = k_ctx->krunqueue.cur_uthread->start_time;
	time_diff = tv2.tv_sec*1000000 + tv2.tv_usec - (tv1.tv_sec*1000000) - (tv1.tv_usec);
	//printf("TIME DIFFERENCE = %lu \n", time_diff );
	int credits;
	credits = (time_diff*25/KTHREAD_VTALRM_USEC);
	printf("CREDITS BURNED BY THE THREAD = %d \n", credits);
...
extern int uthread_create(uthread_t *u_tid, int (*u_func)(void *), void *u_arg, uthread_group_t u_gid, int credits)
{
    ...
    //AOS changed
    u_new->credit = credits;
    u_new->init_credit = 100;
	u_new->start_time.tv_sec = 0;
	u_new->start_time.tv_usec = 0;
	u_new->total_time = 0;
    //AOS change ended
